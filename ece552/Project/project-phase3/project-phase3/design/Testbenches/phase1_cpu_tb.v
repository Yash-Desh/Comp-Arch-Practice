...
module phase1_cpu_tb ();
  ...
  wire [3:0] WriteRegister = DUT.write_reg;  // What 4-bit register number is written
  wire [15:0] WriteData = DUT.write_data;  // 16-bit Data being written to the registerfile.
  wire MemWrite = (DUT.MemWrite);  // Memory is being Written
  wire MemRead = DUT.MemRead;  // Memory is being Read
  wire [15:0] MemAddress = DUT.data_addr;  // 16-bit memory address being accessed
  wire [15:0] MemData = DUT.write_data_mem;  // Data written to Memory on memory writes.
    ...
    if (!rst) begin
      ...
      if (RegWrite) begin
        ...
        $fclose(sim_log_file);
				...
				$writememh("dumpfile_data.img", DUT.data_memory.mem);
        $finish;
      end else begin
        if (MemWrite) begin
          // st
